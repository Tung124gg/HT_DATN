...
<head>
    ...
    <style>
        ...
        /*}*/
        #search-input {
            border: 1px solid #ccc; /* Màu xám nhẹ cho viền */
            border-top-left-radius: 25px; /* Bo góc trên bên trái */
            border-bottom-left-radius: 25px; /* Bo góc dưới bên trái */
            padding: 10px 15px; /* Thêm padding để ô nhập liệu to hơn */
            width: 300px; /* Chiều rộng cố định để ô nhập liệu đẹp hơn */
...
/*        }*/
        #search-btn {
            border: 1px solid #ccc; /* Đổi màu đường viền sang màu xám nhẹ */
            border-top-right-radius: 5px;
            border-bottom-right-radius: 5px;
            border-left: none; /* Không có đường viền bên trái */
            background-color: #f8f9fa; /* Màu nền sáng cho nút */
            color: #007bff; /* Màu chữ xanh */
            padding: 10px 15px; /* Thêm padding để nút to hơn và dễ bấm */
            font-size: 16px; /* Tăng kích thước font chữ */
            cursor: pointer; /* Thêm hiệu ứng con trỏ khi di chuột qua */
            transition: background-color 0.3s, border-color 0.3s; /* Hiệu ứng chuyển màu khi hover */
        }
        ...
        #search-btn:focus {
            ...
            box-shadow: 0 0 5px rgba(0, 123, 255, 0.5); /* Thêm hiệu ứng đổ bóng khi nút được chọn */
        }
        ...
        #reset {
            margin-left: 10px;
            padding: 8px 15px; /* Thêm padding để nút lớn hơn và dễ nhấn */
            background-color: #ffffff; /* Nền trắng */
            border: 1px solid #ccc; /* Viền xám nhẹ để nút không quá nổi bật */
            border-radius: 25px; /* Bo góc tròn để tạo cảm giác mềm mại */
            font-size: 16px; /* Tăng kích thước chữ để dễ đọc */
            color: #007bff; /* Màu chữ xanh để tạo sự nổi bật */
            cursor: pointer; /* Thêm hiệu ứng con trỏ khi di chuột vào nút */
            transition: all 0.3s ease; /* Hiệu ứng chuyển màu mượt mà khi hover */
        }
    ...
    <div class="container">
        ...
        >
            <a
               ...
               class="d-flex align-items-center mb-2 mb-lg-0 text-red text-decoration-none" id="banner">MÃ HÓA ĐƠN :</a>
...
<section class="container-fluid">
    ...
    <div id="cart-detail">
        <label for="search-input" class="form-label" style="color: red">Tìm kiếm sản phẩm:</label> <br>
        <input type="search" name="search-name" id="search-input">
        <button id="search-btn"><i class="bi bi-search"></i></button>
        <button id="reset"><i class="bi bi-arrow-clockwise"></i></button>
...
<%--                                    <p class="card-text price-card-product">${gtt.giayTheThao.giaBan}</p>--%>
                                    <p class="card-text price-card-product">
                                        <fmt:formatNumber value="${gtt.giayTheThao.giaBan}" type="number" groupingUsed="true" /> đ
                                    </p>
                            ...
                            <div id="modal-detailProduct">
                                ...
                                <div id="detailProduct-info">
                                    ...
                                    <p id="detailProduct-color">Màu sắc: <span id="listMS"><span
                                            class="color">Vàng</span> <span class="color">Đỏ</span>
                    ...
                    <tbody id="list-hdc">
                        <c:forEach items="${listHDC}" var="hdc" varStatus="status">
                            <tr onclick="getHDC(`${hdc.id}`)">
                                ...
                                <td>${hdc.maHoaDon}</td>
                                <td>${hdc.user.tenUser==null?"Khách hàng lẻ":hdc.user.tenUser}</td>
...
<%--    <div id="listSP" class="container mt-4">--%>
<%--        <table class="table table-hover table-bordered align-middle">--%>
<%--            <thead class="table-dark text-center">--%>
<%--            <tr>--%>
<%--                <th scope="col">STT</th>--%>
...
<%--            </tr>--%>
<%--            </thead>--%>
<%--&lt;%&ndash;            <tbody id="list-hdc">&ndash;%&gt;--%>
<%--            <c:forEach items="${listHDC}" var="hdc" varStatus="status">--%>
<%--                <tr class="text-center" onclick="getHDC('${hdc.id}')" style="cursor: pointer;">--%>
<%--                    <td>${status.index + 1}</td>--%>
<%--                    <td>${hdc.maHoaDon}</td>--%>
<%--                    <td>${hdc.user.tenUser == null ? "N/A" : hdc.user.tenUser}</td>--%>
<%--                    <td>${hdc.khachHang.tenKhachHang}</td>--%>
<%--&lt;%&ndash;                    <td>&ndash;%&gt;--%>
<%--&lt;%&ndash;                        <button class="btn btn-primary btn-sm" onclick="getHDC('${hdc.id}')">&ndash;%&gt;--%>
...
<%--                            </div>--%>
                            <div class="modal-body">
                                <!-- Tìm kiếm khách hàng -->
                                <div class="mb-3">
                                    <label for="search-client" class="form-label fw-bold">
                                        <i class="bi bi-search"></i> Tìm kiếm khách hàng
                                    </label>
                                    <input
                                            type="search"
                                ...
                                <!-- Bảng khách hàng -->
                                <div id="scroll-kh" class="table-responsive" style="max-height: 300px; overflow-y: auto;">
                                    <table class="table table-hover table-bordered align-middle">
...
<script>
    ...
    function validateQuantity(input) {
        ...
        // Chuyển giá trị thành số nguyên để so sánh
        let value = parseInt(input.value, 10);

        // Kiểm tra giá trị nhỏ hơn 1 hoặc lớn hơn 999
        if (value > 999) {
            document.getElementById('notification-detailProduct').innerText = 'Không nhập số lượng > 999';
        } else if (value < 1) {
            document.getElementById('notification-detailProduct').innerText = 'Không nhập số lượng < 1';
        } else {
            document.getElementById('notification-detailProduct').innerText = '';
        }
    }
    ...
    function toggleColorAndSize() {
        ...
        colors.forEach(function (color) {
            color.onclick = function () {
                ...
                color.style.backgroundColor = 'rgb(104,79,214)';
        ...
        sizes.forEach(function (size) {
            size.onclick = function () {
                ...
                // Đổi màu nền của span được click
                size.style.backgroundColor = 'rgb(104,79,214)';
                kichCo = size.id;
                console.log(kichCo, '-', mauSac, '-', idProduct);
                if (mauSac != '') {
    ...
    function addProductToCart(idGTTCT) {
        var btnThem = document.getElementById('addProductToCart');
        btnThem.addEventListener('click', () => {
            // Kiểm tra nếu chưa chọn màu sắc và kích cỡ
            if (mauSac == '' && kichCo == '') {
                alert('Hãy chọn màu sắc và kích cỡ');
            } else if (mauSac != '' && kichCo == '') {
                alert('Hãy chọn kích cỡ');
                ...
                // Sau khi đã chọn đủ màu sắc và kích cỡ, kiểm tra số lượng
                var quantity = document.getElementById("quantityDetailProduct").value; // Lấy giá trị số lượng

                if (!quantity || quantity <= 0) { // Kiểm tra nếu số lượng trống hoặc không hợp lệ
                    alert('Số lượng không được để trống và phải lớn hơn 0');
                    return; // Dừng hàm nếu số lượng không hợp lệ
                }

                // Tiếp tục lấy thông tin về sản phẩm
                getIdGttct(
                    data => {
                        if (data.soLuong == 0) {
                            idGttct = '';
                        ...
                        document.getElementById("quantityDetailProduct").value = ""; // Xóa số lượng sau khi thêm
                        $('#showDetailProduct').modal('hide'); // Đóng modal chi tiết sản phẩm
                    }
                );
            }
        });
