<%--<%@ page contentType="text/html;charset=UTF-8" language="java" %>--%>
<%--<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>--%>
<%--<%@ taglib prefix="s" uri="http://www.springframework.org/tags" %>--%>
<%--<%@ taglib uri="http://www.springframework.org/tags/form" prefix="frm" %>--%>

<%--<!doctype html>--%>
<%--<html lang="en">--%>
<%--<head>--%>
<%--    <meta charset="utf-8">--%>
<%--    <meta name="viewport" content="width=device-width, initial-scale=1">--%>
<%--    <title>Khách hàng</title>--%>
<%--    &lt;%&ndash;    <link rel="icon" href="img/background_title.jpg" type="image/x-icon">&ndash;%&gt;--%>
<%--    <style>--%>

<%--    </style>--%>
<%--    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet"--%>
<%--          integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">--%>
<%--</head>--%>
<%--<body>--%>
<%--<%@ include file="../templates/Admin/Layouts/GiayTheThao/_HeaderGiayTheThao.jsp" %>--%>
<%--<div class="container">--%>
<%--    <h3 style="text-align: center;">Edit khách hàng</h3>--%>
<%--    <div>--%>
<%--        <h1 style="text-align: center; color: black; margin-top: 30px; margin-bottom: 60px">Cập nhật khách hàng</h1>--%>
<%--        &lt;%&ndash;@elvariable id="khachHang" type=""&ndash;%&gt;--%>
<%--        <frm:form--%>
<%--                modelAttribute="khachHang"--%>
<%--                action="${pageContext.request.contextPath}/KhachHang/create"--%>
<%--                method="POST"--%>
<%--                enctype="multipart/form-data">--%>
<%--            <div class="form-group">--%>
<%--                <frm:hidden path="id" value="${khachHang.id}"/>--%>
<%--            </div>--%>
<%--            <div class="row">--%>
<%--                <div class="col-6">--%>
<%--                    <label style="">Tên khách hàng</label>--%>
<%--                    <br>--%>
<%--                    <frm:input cssStyle="width: 500px; height: 35px;border-radius: 5px 5px 5px" path="tenKhachHang"--%>
<%--                               placeholder="text" value=""/>--%>
<%--                    <frm:errors cssClass="text-danger" path="tenKhachHang"></frm:errors>--%>
<%--                    <label class="">${tenKhachHangNotNull}</label>--%>
<%--                    <label>${tenKhachHangHopLe}</label>--%>
<%--                </div>--%>
<%--                <div class="col-6">--%>
<%--                    <label style="">Email</label>--%>
<%--                    <br>--%>
<%--                    <frm:input cssStyle="width: 500px; height: 35px;border-radius: 5px 5px 5px" path="email"--%>
<%--                               placeholder="text" value=""/>--%>
<%--                    <frm:errors cssClass="text-danger" path="email"></frm:errors>--%>
<%--                    <label class="">${emailNotNull}</label>--%>
<%--                    <label>${emailHopLe}</label>--%>
<%--                </div>--%>
<%--                <div class="col-6">--%>
<%--                    <label style="">Mật khẩu</label>--%>
<%--                    <br>--%>
<%--                    <frm:input cssStyle="width: 500px; height: 35px;border-radius: 5px 5px 5px" path="matKhau"--%>
<%--                               placeholder="text" value=""/>--%>
<%--                    <frm:errors cssClass="text-danger" path="matKhau"></frm:errors>--%>
<%--                    <label class="">${matKhauNotNull}</label>--%>
<%--                </div>--%>
<%--                <div class="col-6">--%>
<%--                    <label style="">Địa chỉ</label>--%>
<%--                    <br>--%>
<%--                    <frm:input cssStyle="width: 500px; height: 35px;border-radius: 5px 5px 5px" path="diaChi"--%>
<%--                               placeholder="text" value=""/>--%>
<%--                    <frm:errors cssClass="text-danger" path="diaChi"></frm:errors>--%>
<%--                    <label class="">${diaChiNotNull}</label>--%>
<%--                    <label>${diaChiHopLe}</label>--%>
<%--                </div>--%>
<%--                <div class="col-6">--%>
<%--                    <label style="">Số điện thoại</label>--%>
<%--                    <br>--%>
<%--                    <frm:input cssStyle="width: 500px; height: 35px;border-radius: 5px 5px 5px" path="soDienThoai"--%>
<%--                               placeholder="text" value=""/>--%>
<%--                    <frm:errors cssClass="text-danger" path="soDienThoai"></frm:errors>--%>
<%--                    <label class="">${soDienThoaiNotNull}</label>--%>
<%--                    <label>${soDienThoaiHopLe}</label>--%>
<%--                </div>--%>
<%--                <div class="col-6">--%>
<%--                    <label style="">Ngày sinh</label>--%>
<%--                    <br>--%>
<%--                    <frm:input cssStyle="width: 500px; height: 35px;border-radius: 5px 5px 5px" path="ngaySinh"--%>
<%--                               type="date" value=""/>--%>
<%--                </div>--%>

<%--                <div class="col-6">--%>
<%--                    <label style="">Ghi chú</label>--%>
<%--                    <br>--%>
<%--                        &lt;%&ndash;                    <frm:textarea id="long-text" name="long-text" rows="4" cols="50"></frm:textarea>&ndash;%&gt;--%>
<%--                    <frm:input cssStyle="width: 500px; height: 35px;border-radius: 5px 5px 5px" path="ghiChu" type=""--%>
<%--                               value=""/>--%>
<%--                        &lt;%&ndash;                    <textarea id="long-text" path="ghiChu" name="long-text" rows="4" cols="50"></textarea>&ndash;%&gt;--%>

<%--                        &lt;%&ndash;                                <div class="textTitle"><frm:errors cssStyle="margin-left: 400px; color: red" class="errors" path="name" /></div>&ndash;%&gt;--%>
<%--                </div>--%>
<%--                <div class="col-6">--%>
<%--                    <label style="">Giới tính</label>--%>
<%--                    <br>--%>
<%--                    <frm:select path="gioiTinh" cssStyle="width: 500px; height: 35px;border-radius: 5px 5px 5px">--%>
<%--                        <option value="Nam">--%>
<%--                            Nam--%>
<%--                        </option>--%>
<%--                        <option value="Nữ">--%>
<%--                            Nữ--%>
<%--                        </option>--%>
<%--                    </frm:select>--%>
<%--                </div>--%>
<%--                <div class="col-6">--%>
<%--                    <label style="">Trạng thái</label>--%>
<%--                    <br>--%>
<%--                    <frm:select path="trangThai" cssStyle="width: 500px; height: 35px;border-radius: 5px 5px 5px">--%>
<%--                        <option value="1">--%>
<%--                            Còn hoạt động ✔--%>
<%--                        </option>--%>
<%--                        <option value="0">--%>
<%--                            Không còn hoạt động ✖--%>
<%--                        </option>--%>
<%--                    </frm:select>--%>
<%--                </div>--%>
<%--            </div>--%>
<%--            <br>--%>
<%--            <br>--%>
<%--            <button type="submit"--%>
<%--                    onclick="if (!confirm('Bạn chắc chắn muốn edit không ?')){return false;}else{return true;}"--%>
<%--                    style="margin-left: 600px; margin-bottom: 190px" class="btn btn-primary">Update--%>
<%--            </button>--%>
<%--            <label>${message}</label>--%>
<%--        </frm:form>--%>
<%--    </div>--%>
<%--</div>--%>
<%--<%@ include file="../templates/Admin/Layouts/GiayTheThao/_FooterGiayTheThao.jsp" %>--%>
<%--<script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.11.8/dist/umd/popper.min.js"--%>
<%--        integrity="sha384-I7E8VVD/ismYTF4hNIPjVp/Zjvgyol6VFvRkX/vR+Vc4jQkC+hVqc2pM8ODewa9r"--%>
<%--        crossorigin="anonymous"></script>--%>
<%--<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.min.js"--%>
<%--        integrity="sha384-BBtl+eGJRgqQAUMxJ7pMwbEyER4l1g+O15P+16Ep7Q9Q+zqX6gSbd85u4mG4QzX+"--%>
<%--        crossorigin="anonymous"></script>--%>
<%--<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js"--%>
<%--        integrity="sha384-C6RzsynM9kWDrMNeT87bh95OGNyZPhcTNXj1NW7RuBCsyN/o0jlpcV8Qyq46cDfL"--%>
<%--        crossorigin="anonymous"></script>--%>
<%--</body>--%>
<%--</html>--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib prefix="s" uri="http://www.springframework.org/tags" %>
<%@ taglib uri="http://www.springframework.org/tags/form" prefix="frm" %>

<!doctype html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Khách hàng</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css" rel="stylesheet"
          integrity="sha384-T3c6CoIi6uLrA9TneNEoa7RxnatzjcDSCmG1MXxSR1GAsXEV/Dwwykc2MPK8M2HN" crossorigin="anonymous">
</head>
<body>
<div class="container">
    <h3 style="text-align: center;">Edit khách hàng</h3>
    <%--@elvariable id="khachHang" type=""--%>
    <frm:form modelAttribute="khachHang" action="" method="POST">
        <div class="form-group">
            <frm:hidden path="id" value="${khachHang.id}"/>
        </div>
        <div class="row">
            <div class="col-6">
                <label for="tenKhachHang">Tên khách hàng</label>
                <frm:input path="tenKhachHang" class="form-control" placeholder="Tên khách hàng"/>
                <frm:errors path="tenKhachHang" cssClass="text-danger"/>
            </div>
            <div class="col-6">
                <label for="email">Email</label>
                <frm:input path="email" class="form-control" placeholder="Email"/>
                <frm:errors path="email" cssClass="text-danger"/>
            </div>
            <div class="col-6">
                <label for="matKhau">Mật khẩu</label>
                <frm:input path="matKhau" class="form-control" placeholder="Mật khẩu"/>
                <frm:errors path="matKhau" cssClass="text-danger"/>
            </div>
            <div class="col-6">
                <label for="diaChi">Địa chỉ</label>
                <frm:input path="diaChi" class="form-control" placeholder="Địa chỉ"/>
                <frm:errors path="diaChi" cssClass="text-danger"/>
            </div>
            <div class="col-6">
                <label for="soDienThoai">Số điện thoại</label>
                <frm:input path="soDienThoai" class="form-control" placeholder="Số điện thoại"/>
                <frm:errors path="soDienThoai" cssClass="text-danger"/>
            </div>
            <div class="col-6">
                <label for="ngaySinh">Ngày sinh</label>
                <frm:input path="ngaySinh" type="date" class="form-control"/>
            </div>
            <div class="col-6">
                <label for="gioiTinh">Giới tính</label>
                <frm:select path="gioiTinh" class="form-control">
                    <option value="Nam">Nam</option>
                    <option value="Nữ">Nữ</option>
                </frm:select>
            </div>
        </div>
        <button type="submit" class="btn btn-primary mt-3">Update</button>
    </frm:form>
</div>
</body>
</html>
